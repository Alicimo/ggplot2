...
from ggplot import aes, geom_bar, ggplot, position_dodge, position_stack


def test_position_stack_produces_ymin_ymax():
    df = pd.DataFrame({'x': ['a', 'a', 'a', 'b', 'b'], 'fill': ['u', 'v', 'u', 'u', 'v']})
...
def test_position_dodge_offsets_x_numeric():
    df = pd.DataFrame({'x': ['a', 'a', 'b', 'b'], 'fill': ['u', 'v', 'u', 'v'], 'y': [1, 1, 1, 1]})
