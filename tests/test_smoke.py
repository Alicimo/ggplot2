...
def test_discrete_scales_and_stats():
    ...
    fig = (ggplot(df, aes('x', 'y', color='g')) + geom_point() + scale_color_manual({'a': 'red', 'b': 'blue'})).draw()
    assert len(fig.data) == 2
...
def test_geoms_smoke():
    assert len((ggplot(pd.DataFrame({'x': [0, 1, 2], 'y': [0, 1, 0]}), aes('x', 'y')) + geom_path()).draw().data) == 1
    assert len((ggplot(pd.DataFrame({'x': [0], 'y': [0], 'xend': [1], 'yend': [1]}), aes('x', 'y', xend='xend', yend='yend')) + geom_segment()).draw().data) == 1
    assert len((ggplot(pd.DataFrame({'x': [0], 'y': [0], 'label': ['hi']}), aes('x', 'y', label='label')) + geom_text()).draw().data) == 1

    df = pd.DataFrame({'xmin': [0], 'xmax': [1], 'ymin': [0], 'ymax': [1]})
    assert len((ggplot(df, aes(xmin='xmin', xmax='xmax', ymin='ymin', ymax='ymax')) + geom_rect()).draw().data) == 1

    df = pd.DataFrame({'x': [0, 1, 2], 'ymin': [0, 0, 0], 'ymax': [1, 2, 1]})
    assert len((ggplot(df, aes('x', ymin='ymin', ymax='ymax')) + geom_ribbon()).draw().data) == 1

    df = pd.DataFrame({'x': [0, 1, 2], 'y': [1, 2, 1]})
    assert len((ggplot(df, aes('x', 'y')) + geom_area()).draw().data) == 1

    fig = (ggplot(pd.DataFrame({'xintercept': [1, 2]}), aes(xintercept='xintercept')) + geom_vline()).draw()
    assert len(fig.layout.shapes) == 2

    assert len((ggplot(pd.DataFrame({'x': [1, 1, 2, 3, 3, 3]}), aes('x')) + geom_histogram(bins=3)).draw().data) == 1
